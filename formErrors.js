...
.directive('form', ['FormErrorsOptions', '$rootScope',
	function (opts, $rootScope) {
		return {
      restrict: 'EAC',
			priority: 1,
			require: ['?form'],
        ...
        if(scope.emitErrors){
  				...
  				var thisCrawlErrors = angular.bind(formCtrl, crawlErrors);
  				var getErrors = function () {
  					var errors = thisCrawlErrors(formCtrl);
		...
		function crawlErrors(ctrl, errors) {
...
.directive('errorMessages', [
	function () {
        ...
        console.log(ctrl);
				...
				if (!ctrl) throw new Error('You need to attach errorMessages to a form!');
